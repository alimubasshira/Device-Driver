...
static wait_queue_head_t wq;   //wait queue where process waits for event
...
static ssize_t sample_read(struct file *filep, char __user *ubuff, size_t cnt, loff_t * offset)
{
	...
	int ret;
	int mini;
	wait_event_interruptible(wq,CIRC_CNT(cbuff.head,cbuff.tail,SIZE)>0); // CIRC_CNT ==0 means no Data
...
static ssize_t sample_write(struct file *filep, const char __user * ubuff, size_t cnt, loff_t *offset)
{
	...
	int ret;
	int mini;
//	wait_event_interruptible(wq,CIRC_SPACE(cbuff.head,cbuff.tail,SIZE)>0); // CIRC_SPACE ==0 means no vacant space;
...
static int __init sample_init(void)
{
	...
	init_waitqueue_head(&wq);// init our waitqueue
